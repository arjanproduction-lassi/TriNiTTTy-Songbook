...
export function parseImportText(text: string): Line[] {
  ...
  for (let i = 0; i < rows.length; i += 1) {
    ...
    if (!trimmed) out.push({ type: "space" });
      ...
      if (canPair) {
        out.push({ type: "pair", chords: line, lyrics: next });
        i += 1;
      } else {
...
export function serializeLines(lines: Line[]) {
  return lines
    .map((line) => {
      ...
      if (line.type === "pair") return `${line.chords}\n${line.lyrics}`;
...
export function convertLine(line: Line, nextType: Line["type"]): Line {
  const fallbackText = line.type === "pair" ? line.lyrics || line.chords : line.type === "space" ? "" : line.text;
  if (nextType === "space") return { type: "space" };
  if (nextType === "section") return { type: "section", text: fallbackText || "Nová sekcia" };
  if (nextType === "pair") {
    return {
      type: "pair",
      chords: line.type === "pair" ? line.chords : line.type === "chords" ? line.text : "",
      lyrics: line.type === "pair" ? line.lyrics : line.type === "lyrics" ? line.text : fallbackText,
    };
  }
  ...
  if (nextType === "chords") return { type: "chords", text: line.type === "pair" ? line.chords : fallbackText };
